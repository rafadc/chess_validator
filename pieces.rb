...
class Rook < MoveCoordinates
  def can_move?
    result = (@origin_col == @destination_col) || (@origin_row == @destination_row)
  end
end

class Bishop < MoveCoordinates
  def can_move?
    result = (@origin_col - @destination_col).abs == (@origin_row - @destination_row).abs
  end
end

class Queen < MoveCoordinates
  def can_move?
    result = Bishop.new(@origin, @destination).can_move? || Rook.new(@origin, @destination).can_move?
  end
end

class King < MoveCoordinates
  def can_move?
    result = (@origin_col - @destination_col).abs <= 1 && (@origin_row - @destination_row).abs <= 1
  end
end
